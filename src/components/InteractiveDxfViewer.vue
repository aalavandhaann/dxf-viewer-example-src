...
import {InteractiveDxfViewer} from "../classes/InteractiveDxfViewer.js"
import InteractiveDxfViewerWorker from "worker-loader!../classes/InteractiveDxfViewerWorker"


/** Events: all DxfViewer supported events (see DxfViewer.Subscribe()), prefixed with "dxf-". */
export default {
    name: "InteractiveDxfViewer",
        ...
        async Load(url) {
            ...
            try {
                await this.dxfViewer.Load({
                    ...
                    workerFactory: InteractiveDxfViewerWorker
                })
            } catch (error) {
                console.warn(error)
    ...
    mounted() {
        this.dxfViewer = new InteractiveDxfViewer(this.$refs.canvasContainer, this.options)
        const Subscribe = eventName => {
            this.dxfViewer.Subscribe(eventName, e => this.$emit("dxf-" + eventName, e, this.dxfViewer))
        }
